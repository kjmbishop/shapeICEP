function tensors = S4Tensor
%TdTensor Prompts user for S4 tensor information and outputs Cijk and Dijk
...
%   plus an example particle shape stored in shapeB
    ...
    tensors.shapeB(4,5) = 0.5;
    tensors.shapeB(8,9) = 0.5;
    tensors.shapeB(2,1) = -0.1;
	tensors.shapeB(3,3) = 0.3;
	...
	tensorDisp(3,1,1).C = '-c1';
    tensorDisp(3,2,2).C = 'c1';
	
    tensorDisp(1,3,1).C = '-c2';
    tensorDisp(1,1,3).C = '-c2';
    tensorDisp(2,3,2).C = 'c2';
    tensorDisp(2,2,3).C = 'c2';
	
    tensorDisp(1,3,2).C = 'c3';
    tensorDisp(1,2,3).C = 'c3';
	tensorDisp(2,3,1).C = 'c3';
	tensorDisp(2,1,3).C = 'c3';
	
	tensorDisp(3,1,2).C = 'c4';
	tensorDisp(3,2,1).C = 'c4';
    ...
    DPrompt = 'Please enter values for [d1, d2, d3, d4]. \n';
    ...
    CPrompt = 'Please enter values for [c1, c2, c3, c4]. \n';
    CArray = input(CPrompt);
    
	tensors.Cijk(3,1,1) = -CArray(1);
    tensors.Cijk(3,2,2) = CArray(1);
	
    tensors.Cijk(1,3,1) = -CArray(2);
    tensors.Cijk(1,1,3) = -CArray(2);
    tensors.Cijk(2,3,2) = CArray(2);
    tensors.Cijk(2,2,3) = CArray(2);
	
    tensors.Cijk(1,3,2) = CArray(3);
    tensors.Cijk(1,2,3) = CArray(3);
	tensors.Cijk(2,3,1) = CArray(3);
	tensors.Cijk(2,1,3) = CArray(3);
	
	tensors.Cijk(3,1,2) = CArray(4);
	tensors.Cijk(3,2,1) = CArray(4);
